# 树莓派 5 + Hailo-8 DeepLab v3 MobileNet v2 语义分割

面向 **Raspberry Pi 5 + Hailo-8**（reComputer R20 系列）的 DeepLab v3 MobileNet v2
...
`deeplab_v3_mobilenet_v2.hef`，放进 `model/`：
...
## 5. REST API
curl -X POST http://localhost:8000/api/models/deeplab/predict -F "file=@cat.jpg"

主要接口（完整列表见项目根 [README_zh.md](../../README_zh.md)）：

- `POST /api/models/deeplab/predict` — 对上传图片、视频帧或当前摄像头帧做一次推理。
...
1. 复制 `src/rpi5_hailo8_unet_mobilenet_v2/` 整个目录并重命名
