...
OBJ_THRESH = 0.25
NMS_THRESH = 0.45
IMG_SIZE = (513, 513)  # (width, height) — must match the .hef input shape

DEFAULT_CLASSES = ("background", "aeroplane", "bicycle", "bird", "boat", "bottle",
...
@app.post("/api/models/deeplab/predict")
async def predict(
...
def post_process_hailo(hailo_output, obj_thresh, nms_thresh, input_h, input_w):
    ...
    Expects output shapes such as (1, 513, 513, num_classes) or (1, num_classes, 513, 513).
    Returns a 2D integer mask of shape (input_h, input_w) containing class indices per pixel.
    """
    if hailo_output is None:
        return None

    # Extract the primary raw output tensor from dictionary
    output_tensor = list(hailo_output.values())[0]
    
    # Remove batch dimension if present, e.g., shape becomes (513, 513, num_classes)
    if len(output_tensor.shape) == 4:
        output_tensor = np.squeeze(output_tensor, axis=0)
        
    # If the channels are at the front (C, H, W), transpose to (H, W, C)
    if output_tensor.shape[0] == len(CLASSES):
        output_tensor = np.transpose(output_tensor, (1, 2, 0))

    # Perform argmax across the channel dimension to get the dominant class index per pixel
    class_mask = np.argmax(output_tensor, axis=-1).astype(np.uint8)
...
def draw_segmentation_mask(image, mask, lb_info):
    ...
    np.random.seed(42)
    max_class_idx = int(np.max(full_scale_mask)) if full_scale_mask.size > 0 else 0
    actual_num_classes = max(len(CLASSES), max_class_idx + 1)

    if max_class_idx >= len(CLASSES):
        print(f"Warning: mask contains class index {max_class_idx}, but only {len(CLASSES)} classes defined. "
              f"Padding color palette to {actual_num_classes} classes.", flush=True)

    colors = np.random.randint(0, 255, size=(actual_num_classes, 3), dtype=np.uint8)
    colors[0] = [0, 0, 0]  # Background remains uncolored

    color_mask = colors[full_scale_mask]

    alpha = 0.4
    mask_indices = full_scale_mask > 0
...
def main():
    parser = argparse.ArgumentParser(description='DeepLab v3 MobileNet v2 semantic segmentation on RPi5 + Hailo-8 (Web Preview Mode)')
