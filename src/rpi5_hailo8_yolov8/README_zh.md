# 树莓派 5 + Hailo-8 YOLOv8 模板

这是面向 **Raspberry Pi 5 + Hailo-8**（reComputer R20 系列）的 YOLOv8 目标检测模块。
按**模板**方式组织：复制目录、换 `.hef`，即可迁移到 Hailo Model Zoo 的其它模型
（yolov8s/m、yolov5、yolov8-seg、yolov8-pose 等）。
...
`yolov8n.hef`，放进 `model/`：

```bash
cd src/rpi5_hailo8_yolov8/model
# 示例路径——具体版本去 Model Zoo 找匹配你 HailoRT 版本的那个
wget https://hailo-model-zoo.s3.eu-west-2.amazonaws.com/ModelZoo/Compiled/<version>/hailo8/yolov8n.hef
```

Model Zoo 默认的 `yolov8n.hef` **内置了 NMS 后处理层**，这正是
...
sudo docker run --rm --privileged --net=host \
    ...
    -v /usr/lib/libhailort.so:/usr/lib/libhailort.so:ro \
    rpi5-hailo8-yolov8:latest \
    python web_detection.py --model_path model/yolov8n.hef --video_path video/test.mp4 --class_path class_config.txt
```
...
python web_detection.py --model_path model/yolov8n.hef --camera_id 0
# 或
python web_detection.py --model_path model/yolov8n.hef --video_path video/test.mp4
```
...
- `POST /api/models/yolov8/predict` — 对上传图片、视频帧或当前摄像头帧做一次推理
...
`nms_thresh` 仍保留以兼容原 API，但 Model Zoo 的 `yolov8n.hef` 已经在芯片上做完
...
## 7. 迁移到其它模型（yolov5、yolov8-seg 等）

把这个目录当模板用：

1. 复制 `src/rpi5_hailo8_yolov8/` 整个目录，重命名为如 `rpi5_hailo8_yolov5/`
