...
Grab a pre-compiled `segformer_b0_bn.hef` from the
...
`(1, 256, 256, 37)`. This is what [web_detection.py](web_detection.py)'s
...
sudo docker run --rm --privileged --net=host \
    ...
    -v /usr/lib/libhailort.so:/usr/lib/libhailort.so:ro \
    rpi5-hailo8-segformer:latest \
    python web_detection.py --model_path segformer_b0_bn.hef --camera_id 0
```
...
- `POST /api/models/segformer/predict` — single-shot inference on uploaded image, video
...
1. Copy the whole `src/rpi5_hailo8_segformer_b0_bn/` folder and rename it.
