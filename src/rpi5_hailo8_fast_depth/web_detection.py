...
OBJ_THRESH = 0.65
NMS_THRESH = 0.45
IMG_SIZE = (244, 244)  # (width, height); overwritten from the HEF at startup.
DEPTH_OVERLAY_ALPHA = 0.65
_DEPTH_OUTPUT_LOGGED = False

CLASSES = ("depth",)

def load_classes(path):
    if path:
        print("FastDepth is a depth-estimation model; --class_path is ignored.", flush=True)

class DetectionConfig:
    def __init__(self):
        self.obj_thresh = 0.65
            ...
            while not self._stop_event.is_set():
                ...
                if self.model and self.co_helper:
                    ...
                    if outputs is not None:
                        depth_map = post_process_hailo(outputs, 0, 0, IMG_SIZE[1], IMG_SIZE[0])
                        if depth_map is not None:
                            draw_depth_map(frame, depth_map, lb_info)
                if kind == 'ffmpeg':
...
app = FastAPI(title="reComputer FastDepth Depth Estimation (RPi5 + Hailo-8)")
...
async def update_config(config: dict):
    det_config.update(config.get("obj_thresh", 0.65), config.get("nms_thresh", 0.45))
...
@app.post("/api/models/fast_depth/predict")
async def predict(
    ...
    try:
        ...
        depth = None
        if depth_map is not None:
            depth = depth_to_original(depth_map, lb_info, img.shape[:2])
            draw_depth_map(img, depth_map, lb_info)
        ...
        return {
            ...
            "depth": depth_statistics(depth) if depth is not None else None,
            "image": {
                "width": w,
                "height": h
            }
...
async def index():
    ...
    <html>
      <head>
        <title>reComputer FastDepth Depth Estimation</title>
        <style>
      ...
      <body>
        <div class="container">
          <h1>RPi5 + Hailo-8 Depth Estimation</h1>

          <div class="tabs">
            <div class="tab active" onclick="showTab('realtime')">Real-time Depth</div>
          ...
          <div id="realtime" class="tab-content active">
            ...
            <div class="controls">
              <div class="control-group">
                <label>Depth Overlay Intensity</label>
                <div class="slider-container">
                  <input type="range" id="confSlider" min="0.01" max="1.0" step="0.01" value="0.65">
                  <span id="confValue" class="value-display">0.65</span>
                </div>
              </div>

              <div class="control-group">
                <label>Reserved Display Control</label>
                <div class="slider-container">
...
def post_process_hailo(hailo_output, obj_thresh, nms_thresh, input_h, input_w):
    """
    Decode FastDepth output into a 2D relative-depth map.

    FastDepth outputs a continuous depth / inverse-depth style map. This module
    keeps the result as relative depth for display and statistics. If the HEF
    returns logits, they are squashed to a positive [0, 1] range first.
    """
    global _DEPTH_OUTPUT_LOGGED

    if hailo_output is None:
        return None

    output_tensor = np.asarray(_primary_output_tensor(hailo_output), dtype=np.float32)
    ...
    if output_tensor.ndim == 3:
        if output_tensor.shape[-1] == 1:
            ...
            output_tensor = output_tensor.mean(axis=-1)

    if output_tensor.ndim != 2:
        raise ValueError(f"Unsupported FastDepth output shape: {raw_shape}")

    if output_tensor.shape == (input_w, input_h) and input_h != input_w:
        output_tensor = output_tensor.T
    elif output_tensor.shape != (input_h, input_w):
        output_tensor = cv2.resize(output_tensor, (input_w, input_h), interpolation=cv2.INTER_LINEAR)

    finite_raw = output_tensor[np.isfinite(output_tensor)]
    if finite_raw.size and (float(np.min(finite_raw)) < 0.0 or float(np.max(finite_raw)) > 1.5):
        depth = _sigmoid(output_tensor)
    else:
        depth = output_tensor
    depth = np.nan_to_num(depth, nan=0.0, posinf=0.0, neginf=0.0).astype(np.float32)

    if not _DEPTH_OUTPUT_LOGGED:
        finite = depth[np.isfinite(depth) & (depth > 0)]
        stats = "empty"
        if finite.size:
            stats = f"min={float(finite.min()):.4f}, max={float(finite.max()):.4f}, mean={float(finite.mean()):.4f}"
        print(
            f"[FastDepth] raw output shape={raw_shape}, decoded depth shape={depth.shape}, {stats}",
            flush=True,
        )
        _DEPTH_OUTPUT_LOGGED = True
...
def depth_to_original(depth, lb_info, output_shape):
    if depth is None:
        return None
    ...
    content_depth = depth[top:h_end, left:w_end]
    return cv2.resize(content_depth, (out_w, out_h), interpolation=cv2.INTER_LINEAR)
...
def draw_depth_map(image, depth, lb_info):
    if depth is None:
        return image

    full_depth = depth_to_original(depth, lb_info, image.shape[:2])
    color_depth = colorize_depth(full_depth)
    alpha, _ = det_config.get()
    alpha = float(np.clip(alpha if alpha is not None else DEPTH_OVERLAY_ALPHA, 0.0, 1.0))
    cv2.addWeighted(color_depth, alpha, image, 1.0 - alpha, 0, dst=image)
    return image
...
def preprocess_frame(frame, co_helper):
    ...
    exact ratio + padding used for this frame so the depth map can be un-letterboxed
...
def main():
    parser = argparse.ArgumentParser(description='FastDepth monocular depth estimation on RPi5 + Hailo-8 (Web Preview Mode)')
    ...
    # Pull the real (H, W) out of the .hef so non-square depth models work without editing constants. Stored as (W, H)
