# 树莓派 5 + Hailo-8 YOLOv11 模板

这是面向 **Raspberry Pi 5 + Hailo-8**（reComputer R20 系列）的 YOLOv11 目标检测模块。
按**模板**方式组织：复制目录、换 `.hef`，即可迁移到 Hailo Model Zoo 的其它模型
（yolov11n/s/m、yolov8、yolov5、yolov11-seg 等）。
...
Model Zoo 默认的 `yolov11n.hef` **内置了 NMS 后处理层**，这正是
...
- `POST /api/models/yolov11/predict` — 对上传图片、视频帧或当前摄像头帧做一次推理
...
`nms_thresh` 仍保留以兼容原 API，但 Model Zoo 的 `yolov11n.hef` 已经在芯片上做完
...
## 7. 迁移到其它模型（yolov8、yolov5、yolov11-seg 等）

把这个目录当模板用：

1. 复制 `src/rpi5_hailo8_yolov11/` 整个目录，重命名为如 `rpi5_hailo8_yolov5/`
...
- Hailo 推理 8.5 ms/帧（yolov11n.hef）
