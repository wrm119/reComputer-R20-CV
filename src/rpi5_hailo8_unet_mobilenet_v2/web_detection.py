...
OBJ_THRESH = 0.25
NMS_THRESH = 0.45
IMG_SIZE = (256, 256)  # (width, height) must match the .hef input shape

# Oxford-IIIT Pet trimap labels after the TensorFlow tutorial's mask -= 1 step:
# 0 = pet, 1 = border, 2 = surrounding/background.
DEFAULT_CLASSES = ("pet", "border", "background")
BACKGROUND_CLASS_ID = 2

PET_PALETTE = np.array([
    [60, 180, 75],    # pet
    [0, 215, 255],    # border
    [0, 0, 0],        # background
], dtype=np.uint8)
        ...
        with open(path, 'r', encoding='utf-8') as f:
            ...
            if items:
                ...
                if items:
                    ...
                    print(f"Warning: No classes found in {path}, using default Oxford-IIIT Pet classes")
                    CLASSES = DEFAULT_CLASSES
    except Exception as e:
        print(f"Error loading classes from {path}: {e}. Using default Oxford-IIIT Pet classes")
        CLASSES = DEFAULT_CLASSES
...
@app.post("/api/models/unet/predict")
async def predict(
    ...
    try:
        ...
        if mask is not None:
            ...
            for cl, n in zip(class_ids, counts):
                if cl == BACKGROUND_CLASS_ID or cl >= len(CLASSES):
                    continue
                predictions.append({
...
def post_process_hailo(hailo_output, obj_thresh, nms_thresh, input_h, input_w):
    """
    Decode Oxford-IIIT Pet U-Net output.

    The expected model output is a 3-channel per-pixel score map such as
    (256, 256, 3) or (1, 3, 256, 256). The returned mask is a 2D class-index
    array where 0=pet, 1=border, 2=background.
    """
    ...
    if output_tensor.ndim == 2:
        ...
        if output_tensor.shape[-1] == len(CLASSES):
            class_mask = np.argmax(output_tensor, axis=-1)
        elif output_tensor.shape[0] == len(CLASSES):
            ...
            raise ValueError(f"Unsupported Oxford-IIIT Pet output shape: {raw_shape}")
    else:
        raise ValueError(f"Unsupported Oxford-IIIT Pet output shape: {raw_shape}")
    ...
    class_mask = np.nan_to_num(class_mask, nan=BACKGROUND_CLASS_ID,
                               posinf=BACKGROUND_CLASS_ID,
                               neginf=BACKGROUND_CLASS_ID)
    class_mask = np.rint(class_mask).clip(0, len(CLASSES) - 1).astype(np.uint8)

    if not _SEG_OUTPUT_LOGGED:
        unique = np.unique(class_mask)
        print(
            f"[UNet Pet] raw output shape={raw_shape}, decoded mask shape={class_mask.shape}, "
...
def draw_segmentation_mask(image, mask, lb_info):
    ...
    palette_len = min(len(PET_PALETTE), len(colors))
    colors[:palette_len] = PET_PALETTE[:palette_len]
...
def main():
    parser = argparse.ArgumentParser(description='UNNet MobileNet v2 semantic segmentation on RPi5 + Hailo-8 (Web Preview Mode)')
