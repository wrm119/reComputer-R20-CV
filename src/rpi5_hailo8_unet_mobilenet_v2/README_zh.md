# 树莓派 5 + Hailo-8 unet v3 MobileNet v2 语义分割

面向 **Raspberry Pi 5 + Hailo-8**（reComputer R20 系列）的 unet v3 MobileNet v2
...
`unet_mobilenet_v2.hef`，放进 `model/`：
...
## 5. REST API
curl -X POST http://localhost:8000/api/models/unet/predict -F "file=@cat.jpg"

主要接口（完整列表见项目根 [README_zh.md](../../README_zh.md)）：

- `POST /api/models/unet/predict` — 对上传图片、视频帧或当前摄像头帧做一次推理。
