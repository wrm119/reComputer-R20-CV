        ...
        with open(path, 'r', encoding='utf-8') as f:
            ...
            if items:
                ...
                if items:
                    ...
                    print(f"Warning: No classes found in {path}, using default face class")
                    CLASSES = DEFAULT_CLASSES
    except Exception as e:
        print(f"Error loading classes from {path}: {e}. Using default face class")
        CLASSES = DEFAULT_CLASSES
            ...
            while not self._stop_event.is_set():
                ...
                if self.model and self.co_helper:
                    ...
                    if outputs is not None:
                        ...
                        boxes, classes, scores, landmarks = post_process_hailo(
                            outputs, obj, nms, IMG_SIZE[1], IMG_SIZE[0]
                        )
                        if boxes is not None:
                            real_boxes = self.co_helper.get_real_box(boxes)
                            real_landmarks = unletterbox_landmarks(landmarks, self.co_helper)
...
app = FastAPI(title="reComputer SCRFD Face Detection (RPi5 + Hailo-8)")
...
@app.post("/api/models/scrfd/predict")
async def predict(
    ...
    try:
        ...
        if boxes is not None and len(boxes) > 0:
            ...
            real_landmarks = unletterbox_landmarks(landmarks, _global_co_helper)
            for idx, (box, score, cl) in enumerate(zip(real_boxes, scores, classes)):
                points = []
                if real_landmarks is not None and idx < len(real_landmarks):
                    points = [
                        {"x": int(x), "y": int(y)}
                        for x, y in real_landmarks[idx]
...
async def index():
    ...
    <html>
      <head>
        <title>reComputer SCRFD Face Detection</title>
        <style>
      ...
      <body>
        <div class="container">
          <h1>RPi5 + Hailo-8 Face Detection</h1>

          <div class="tabs">
            <div class="tab active" onclick="showTab('realtime')">Real-time Face Detection</div>
          ...
          <div id="realtime" class="tab-content active">
            ...
            <div class="controls">
              ...
              <div class="control-group">
                <label>IOU Threshold (SCRFD post-process NMS)</label>
                <div class="slider-container">
...
def _squeeze_batch(output):
    output = np.asarray(output)
    if output.ndim == 4 and output.shape[0] == 1:
        output = output[0]
    if output.ndim == 3 and output.shape[0] in (1, 4, 8, 10, 20):
        output = np.moveaxis(output, 0, -1)
    return output


def _flatten_scrfd_output(output):
    output = _squeeze_batch(output)
    if output.ndim == 3:
        h, w, c = output.shape
        return output.reshape(h * w, c), h, w, c
    if output.ndim == 2:
        return output, None, None, output.shape[-1]
    return output.reshape(-1, output.shape[-1]), None, None, output.shape[-1]


def _pick_scrfd_outputs(hailo_output):
    branches = {}
    for name, tensor in hailo_output.items():
        flat, h, w, c = _flatten_scrfd_output(tensor)
        if h not in (80, 40, 20):
            continue

        entry = branches.setdefault(h, {})
        if c in (1, 2):
            entry["score"] = flat.reshape(-1)
        elif c in (4, 8):
            entry["bbox"] = flat.reshape(-1, 4)
        elif c in (10, 20):
            entry["landmark"] = flat.reshape(-1, 5, 2)
        else:
            print(f"[SCRFD] Ignoring unexpected output {name}: shape={np.asarray(tensor).shape}", flush=True)

    return branches


def _scrfd_anchor_centers(input_h, input_w, stride):
...
def _scrfd_decode_boxes(raw_boxes, centers):
    x1 = centers[:, 0] - raw_boxes[:, 0]
    y1 = centers[:, 1] - raw_boxes[:, 1]
    x2 = centers[:, 0] + raw_boxes[:, 2]
    y2 = centers[:, 1] + raw_boxes[:, 3]
    return np.stack((x1, y1, x2, y2), axis=-1)


def _scrfd_decode_landmarks(raw_landmarks, centers):
    points = raw_landmarks.copy()
...
def post_process_hailo(hailo_output, obj_thresh, nms_thresh, input_h, input_w):
    """
    Decode SCRFD face-detection outputs from Hailo Model Zoo HEFs.

    SCRFD emits raw score, bbox and landmark heads for strides 8, 16 and 32.
    Boxes and landmarks use the standard distance-to-anchor-center encoding.
    """
    if hailo_output is None:
        return None, None, None, None

    branches = _pick_scrfd_outputs(hailo_output)
    all_boxes, all_scores, all_landmarks = [], [], []

    for stride in SCRFD_STRIDES:
        feat_h = input_h // stride
        branch = branches.get(feat_h)
        if not branch or "score" not in branch or "bbox" not in branch:
            continue

        scores = branch["score"].astype(np.float32)
        bbox_raw = branch["bbox"].astype(np.float32)
        centers = _scrfd_anchor_centers(input_h, input_w, stride)
        count = min(len(scores), len(bbox_raw), len(centers))
        scores, bbox_raw, centers = scores[:count], bbox_raw[:count], centers[:count]

        keep = np.where(scores >= obj_thresh)[0]
        if keep.size == 0:
            continue

        boxes = _scrfd_decode_boxes(bbox_raw[keep] * stride, centers[keep])
        all_boxes.append(boxes)
        all_scores.append(scores[keep])

        landmark_raw = branch.get("landmark")
        if landmark_raw is not None:
            landmark_raw = landmark_raw.astype(np.float32)[:count]
            all_landmarks.append(_scrfd_decode_landmarks(landmark_raw[keep] * stride, centers[keep]))
        else:
            all_landmarks.append(np.full((len(keep), 5, 2), np.nan, dtype=np.float32))
    ...
    if MAX_NMS_CANDIDATES > 0 and len(scores) > MAX_NMS_CANDIDATES:
        top = np.argpartition(scores, -MAX_NMS_CANDIDATES)[-MAX_NMS_CANDIDATES:]
        boxes = boxes[top]
        scores = scores[top]
        landmarks = landmarks[top]
...
def unletterbox_landmarks(landmarks, co_helper):
    if landmarks is None or not hasattr(co_helper, "letter_box_info_list"):
        return landmarks
        ...
        while not stop_event.is_set():
            ...
            if outputs is not None:
                ...
                if boxes is not None:
                    ...
                    real_landmarks = unletterbox_landmarks(landmarks, co_helper)
                    draw(frame, real_boxes, scores, classes, real_landmarks)
...
def main():
    parser = argparse.ArgumentParser(description='SCRFD face detection on RPi5 + Hailo-8 (Web Preview Mode)')
    ...
    parser.add_argument('--max_candidates', type=int, default=1000, help='Maximum SCRFD boxes passed to CPU NMS. Lower values reduce latency. Default 1000.')
    ...
    global MAX_NMS_CANDIDATES
    MAX_NMS_CANDIDATES = args.max_candidates
