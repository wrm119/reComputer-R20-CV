# YOLOv5 on Raspberry Pi 5 + Hailo-8

This is the YOLOv5 object detection module for **Raspberry Pi 5 + Hailo-8**
(reComputer R20 series). It's designed as a **template**: copy this directory and
swap the `.hef` to adapt to other Hailo Model Zoo models (yolov5s/m, yolov8,
yolov11, yolov5-seg, etc.).
...
Grab a pre-compiled `yolov5s.hef` from the
...
The `yolov5s.hef` shipped by the Model Zoo has the **NMS post-process layer baked
...
- `POST /api/models/yolov5/predict` — single-shot inference on uploaded image, video
...
`yolov5s.hef` NMS is already done on-chip, so this slider acts only as an
...
## 7. Adapting to other models (yolov8, yolov11, yolov5-seg, etc.)

Use this directory as the template:

1. Copy the whole `src/rpi5_hailo8_yolov5/` folder, rename to e.g.
   `rpi5_hailo8_yolov5/`.
...
- Hailo inference: 8.5 ms/frame (yolov5s.hef)
