...
@app.post("/api/models/yolov5/predict")
async def predict(
...
def post_process_hailo(hailo_output, obj_thresh, nms_thresh, input_h, input_w):
    """
    Decode the Hailo Model Zoo yolov5 .hef output (built-in NMS post-process layer).
...
def main():
    parser = argparse.ArgumentParser(description='YOLOv5 object detection on RPi5 + Hailo-8 (Web Preview Mode)')
