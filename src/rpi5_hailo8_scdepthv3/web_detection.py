...
OBJ_THRESH = 0.65
NMS_THRESH = 0.45
IMG_SIZE = (320, 256)  # (width, height); overwritten from the HEF at startup.
...
def load_classes(path):
    if path:
        print("SCDepthV3 is a depth-estimation model; --class_path is ignored.", flush=True)
...
app = FastAPI(title="reComputer SCDepthV3 Depth Estimation (RPi5 + Hailo-8)")
...
@app.post("/api/models/scdepthv3/predict")
async def predict(
...
async def index():
    ...
    <html>
      <head>
        <title>reComputer SCDepthV3 Depth Estimation</title>
        <style>
...
def post_process_hailo(hailo_output, obj_thresh, nms_thresh, input_h, input_w):
    """
    Decode SCDepthV3 output into a 2D relative-depth map.

    Hailo's SCDepthV3 sample converts raw network output to relative depth with:
        depth = 1 / (sigmoid(raw) * 10 + 0.009)
    The result is relative depth, not metric meters.
    """
    ...
    if output_tensor.ndim != 2:
        raise ValueError(f"Unsupported SCDepthV3 output shape: {raw_shape}")
    ...
    depth = 1.0 / (_sigmoid(output_tensor) * 10.0 + 0.009)
    ...
    if not _DEPTH_OUTPUT_LOGGED:
        ...
        print(
            f"[SCDepthV3] raw output shape={raw_shape}, decoded depth shape={depth.shape}, {stats}",
            flush=True,
        )
...
def main():
    parser = argparse.ArgumentParser(description='SCDepthV3 monocular depth estimation on RPi5 + Hailo-8 (Web Preview Mode)')
